 ...
 * para el componente `UpdateAdComponent`.
 *
 * @author Robert Ene
 */
describe('UpdateAdComponent', () => {
    ...
    const createUpdateAdComponent = createComponentFactory({
    ...
    let spectator: Spectator<UpdateAdComponent>;
    ...
    beforeEach(() => {
        spectator = createUpdateAdComponent();
    ...
    it(`debería instanciar el componente (título 'LostPets: Modificar Anuncio')`, async () => {
        expect(spectator.component).toBeTruthy();
        expect(spectator.inject(Title).getTitle()).toEqual('LostPets: Modificar Anuncio');
    });


    describe('recuperación del detalle del anuncio para la modificación', () => {

        it(`debería recuperar los detalles de un determinado anuncio para la modificación
            (geolocalización habilitada, anuncio perteneciente al usuario en sesión, foto
            del anuncio cargada, calendario inicializado, mapa inicializado y centrado en
            la ubicación del usuario en sesión)`, async () => {
                const petImageFile = new File([ad.photo], ad.photo);
                ...
                spectator.inject(AdService).getPetImage.andReturn(Promise.resolve(petImageFile));
                ...
                expect(componentSpy.petImageUrl).toEqual(`${apiUrl}/${ad.photo}`);
                expect(componentSpy.petImage).toEqual(petImageFile);
                expect(componentSpy.esCalendar).toBeDefined();
                expect(componentSpy.date).toEqual(new Date(ad.date));
                expect(componentSpy.minDate).toEqual(componentSpy.date);
                ...
                expect(spectator.component.updateAdButtonState()).toBeFalsy();
        ...
        it(`debería no recuperar el detalle del anuncio (código inexistente)`, async () => {
            (spectator.component as any).route = activatedRouteMock;
            activatedRouteMock.snapshot.params.code = undefined;
            spectator.inject(UserService).getLoggedUser.andReturn(Promise.resolve(user));
            ...
            expect(componentSpy.ad).toBeUndefined();
            expect(componentSpy.petImageUrl).toBeUndefined();
            expect(componentSpy.petImage).toBeUndefined();
            expect(componentSpy.esCalendar).toBeUndefined();
            expect(componentSpy.date).toBeUndefined();
            expect(componentSpy.minDate).toBeUndefined();
            expect(componentSpy.maxDate).toBeUndefined();
            expect(componentSpy.checkValidAd()).toBeFalsy();
            expect(spectator.component.updateAdButtonState()).toBeTruthy();
    ...
    describe('modificación de los detalles de un determinado anuncio', () => {
        ...
        it(`debería permitir la modificación de los datos de un determinado anuncio
            ya cargado (administrador en sesión, datos válidos, última ubicación modificada,
                fecha de la desaparición modificada)`, async () => {
                ...
                spyOn(navigator.geolocation, 'getCurrentPosition')
                    .and.callFake(successCallback => successCallback(geolocationPositionMock));
                spectator.inject(UserService).getLoggedUser.andReturn(Promise.resolve(admin));
                spectator.inject(AdService).getAdDetail.andReturn(Promise.resolve(ad));
                spectator.inject(AdService).getPetImage.andReturn(Promise.resolve(petImageFile));
                spectator.inject(AdService).updateAd.andReturn(Promise.resolve(true));

                await spectator.component.ngOnInit();
                queryParamsMock.next({ from: '/ad-detail' });
                spectator.component.onMapReady(googleMapMock);

                let componentSpy = spectator.component as any;
                expect(spectator.inject(SpinnerService).showSpinner).toHaveBeenCalled();
                expect(componentSpy.previousRoute).toEqual('/ad-detail');
                expect(componentSpy.geolocationEnabled).toBeTruthy();
                expect(componentSpy.currentGeolocation).toEqual(geolocationPositionMock);
                ...
                const oldAdDate = componentSpy.date as Date;
                const newAdDate = new Date(componentSpy.minDate);
                newAdDate.setDate(newAdDate.getDate() + 1);
                newAdDate.setHours(oldAdDate.getHours(), oldAdDate.getMinutes(),
                    oldAdDate.getSeconds(), oldAdDate.getMilliseconds());
                spectator.component.onSelectAdDate(newAdDate);
                ...
                componentSpy = spectator.component as any;
                expect(componentSpy.date).toEqual(newAdDate);
                expect(spectator.component.checkValidTime()).toBeTruthy();
                expect(spectator.component.checkInvalidTime()).toBeFalsy();
                expect(componentSpy.checkValidAd()).toBeTruthy();
                expect(spectator.component.updateAdButtonState()).toBeFalsy();

                await spectator.component.updateAd();

                expect(spectator.inject(SpinnerService).showSpinner).toHaveBeenCalled();
                expect(routerNavigateSpy).toHaveBeenCalledWith(['/ad-detail'],
                    { queryParams: { updatedAd: true, code: ad.code } });
                expect(spectator.inject(SpinnerService).hideSpinner).toHaveBeenCalled();
        ...
        it(`debería no permitir la modificación de los datos de un determinado anuncio ya cargado
            (no pertenece al usuario en sesión, el administrador no está en sesión) `, async () => {
                const other = Object.assign({}, user);
                other.id = 21234;
                (spectator.component as any).route = activatedRouteMock;
                spectator.inject(UserService).getLoggedUser.andReturn(Promise.resolve(other));
                spectator.inject(AdService).getAdDetail.andReturn(Promise.resolve(ad));
        ...
        it(`debería no permitir la modificación de los datos de un determinado anuncio ya cargado
            (geolocalización inhabilitada, error al obtener la localización del usuario, datos inválidos)`, async () => {
                (spectator.component as any).route = activatedRouteMock;
        ...
        it(`debería no permitir la modificación de los datos de un determinado anuncio ya cargado
            (error al cargar la actual foto del anuncio, carga de nueva foto para el anuncio,
            cambio de la fecha del anuncio, error en la geolocalización del usuario en sesión) `, async () => {
                const serverError = {
                    error: {
                        field: 'campo',
                        code: '400',
                ...
                spectator.inject(UserService).getLoggedUser.andReturn(Promise.resolve(user));
                spectator.inject(AdService).getAdDetail.andReturn(Promise.resolve(ad));
                spectator.inject(AdService).getPetImage.andReturn(Promise.reject(serverError));
                ...
                getElementsByNameSpy.withArgs('time').and.returnValue([{}, undefined] as any);
                (spectator.component as any).date = (spectator.component as any).minDate;
                spectator.detectChanges();
        ...
        it(`debería no permitir la modificación de los datos personales de
            la cuenta del usuario (error producido en el servidor)`, async () => {
                const serverError = {
                    error: {
                        field: 'campo',
                        code: '400',
                ...
                spectator.inject(AdService).getAdDetail.andReturn(Promise.resolve(ad));
                spectator.inject(AdService).getPetImage.andReturn(Promise.resolve(petImageFile));
                spectator.inject(AdService).updateAd.andReturn(Promise.reject(serverError));

                await spectator.component.ngOnInit();
                queryParamsMock.next({ from: '/ad-detail' });
                spectator.component.onMapReady(googleMapMock);

                (spectator.component as any).ad.petStatus = PetStatus.LOST;
                (spectator.component as any).ad.pet.name = 'Otto';
                (spectator.component as any).ad.reward = 100.00;
                spectator.detectChanges();
